...
import { requestLocationOnClick } from '../gps';
...
const Header = () => {
  ...
  // ✅ Capacitor GPS handler
  const handleGPS = async () => {
    setLocating(true);

    const location = await requestLocationOnClick();
    ...
    toast.success(
      `${t('অবস্থান পাওয়া গেছে', 'Location found')}: ${upazilaName}, ${zillaName}`
    );
  ...
  return (
    <header
      className="sticky top-0 z-50 bg-primary text-primary-foreground shadow-lg md:bg-background md:text-foreground md:shadow-sm md:border-b md:border-border"
      style={{ paddingTop: 'env(safe-area-inset-top)' }}
    >
      <div className="flex items-center gap-3 px-4 py-3">
        <div
          className="flex items-center gap-3 flex-1 md:cursor-pointer"
          onClick={() => navigate('/')}
        >
          <img
            src={logo}
          ...
          <div>
            <h1 className="text-lg font-bold leading-tight md:text-primary">
              {isHome
                ? t('রামাদান ইনসাইট', 'Ramadan Insight')
                : pageTitle
                ? t(pageTitle.bn, pageTitle.en)
                : t('রামাদান ইনসাইট', 'Ramadan Insight')}
            </h1>
